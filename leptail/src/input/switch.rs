...
pub fn Switch(
    ...
    #[prop(into)]
    is_on: MaybeSignal<bool>,
    /// callback when state changes
    #[prop(into, optional)]
    set_on: Option<Out<bool>>,
    /// disabled property; if disabled, click event will have no effect
    #[prop(into, optional)]
    disabled: OptionalMaybeSignal<bool>,
    /// tab_index property;  Optional tab index; if not set, then tabindex will be by default 0
    #[prop(into, optional)]
    tab_index: OptionalMaybeSignal<i32>,
    /// optional on icon
    #[prop(into, optional)]
    on_icon: OptionalMaybeSignal<icondata::Icon>,
    /// optional off icon
    #[prop(into, optional)]
    off_icon: OptionalMaybeSignal<icondata::Icon>,
    /// optional swith theme variant
    #[prop(into, optional)]
    variant: OptionalMaybeSignal<SwitchTheme>,
) -> impl IntoView {
    let theme = variant.or_else(move || use_context::<SwitchTheme>().unwrap_or_default());
    ...
    let switch_modifier = {
        let cloned = theme.clone();
        move || {
            with!(|is_on, is_disabled, cloned| format!(
                "{} {} {}",
                cloned.base.switch,
                if *is_on {
                    cloned.on_modifier.switch.clone()
                } else {
                    cloned.off_modifier.switch.clone()
                },
                if *is_disabled {
                    cloned.disabled_modifier.switch.clone()
                } else {
                    cloned.enabled_modifier.switch.clone()
                }
            ))
        }
    };
    ...
    let icon_modifier = {
        ...
        move || {
            with!(|is_on, is_disabled, cloned| format!(
                ...
                if *is_on {
                    cloned.on_modifier.icon_container.clone()
                } else {
                    cloned.off_modifier.icon_container.clone()
                },
                if *is_disabled {
                    cloned.disabled_modifier.icon_container.clone()
                } else {
                    cloned.enabled_modifier.icon_container.clone()
                }
            ))
        }
    };
    ...
    let toggle_state = move || {
        ...
        if !is_disabled {
            if let Some(set) = &set_on {
                set.set(!is_on.get_untracked())
            }
        }
    };
