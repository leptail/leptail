...
impl DrawerVariant {
    ...
    fn base_theme() -> DrawerTheme {
        ...
        DrawerTheme {
            wrapper: "z-[101] fixed overflow-x-hidden transition-all duration-300 ease-out"
                .to_string(),
    ...
    fn gen_temprovary_drawer(side: &Side, size: &Size, has_inset: bool) -> DrawerTheme {
        ...
        let inset_inner_class = if has_inset {
    ...
    pub fn default_variant() -> DrawerTheme {
        let dv = DrawerVariant::Temporary {
            size: Size::Medium,
            side: Side::Left,
            has_inset: false,
        };
    ...
    pub fn variant(variant: &DrawerVariant) -> DrawerTheme {
        match variant {
            ...
            DrawerVariant::Staggered {
                breakover_point: _,
                side,
            } => Self::gen_staggered_drawer(side, false),
            DrawerVariant::StaggeredMini {
                breakover_point: _,
                side,
            } => Self::gen_staggered_drawer(side, true),
        }
    }
}
